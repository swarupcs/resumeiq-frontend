  ...
  return (
    <div className='space-y-4'>
      ...
      <textarea
        ...
        className='w-full p-3 px-4 border text-sm border-border rounded-lg bg-background text-foreground focus:ring-2 focus:ring-ring outline-none transition-colors resize-none'
      />
    </div>
  );
};
...
export const ExperienceForm = ({
  ...
  const handleEnhance = async (i: number) => {
    ...
    if (result.data?.enhancedContent)
      update(i, 'description', result.data.enhancedContent);
  };
  return (
            ...
            >
              ...
              <div className='grid md:grid-cols-2 gap-3'>
                {(['company', 'position'] as const).map((f) => (
                  <input
                    ...
                    placeholder={f === 'company' ? 'Company Name' : 'Job Title'}
              ...
              <div className='flex items-center gap-2'>
                <Checkbox
                  ...
                  onCheckedChange={(c) => update(i, 'is_current', c)}
                />
                <label
                  htmlFor={`current-${i}`}
              ...
              <div className='space-y-2'>
                <div className='flex items-center justify-between'>
                  ...
                  <button
                    onClick={() => void handleEnhance(i)}
                    disabled={isPending || !exp.position || !exp.company}
            ...
            >
              ...
              <div className='grid md:grid-cols-2 gap-3'>
                ...
                <input
                  ...
                  onChange={(e) => update(i, 'graduation_date', e.target.value)}
                  type='month'
                  className='px-3 py-2 text-sm border border-border rounded-lg bg-background text-foreground focus:ring-2 focus:ring-ring outline-none'
                />
              </div>
...
export const ProjectForm = ({
  ...
  const remove = (i: number) => onChange(data.filter((_, idx) => idx !== i));
  const update = (i: number, field: keyof ProjectEntry, value: string) => {
    const u = [...data];
    u[i] = { ...u[i]!, [field]: value };
    onChange(u);
  };
