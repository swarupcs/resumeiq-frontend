...
const TEMPLATE_MAP: Record<
  string,
  React.ComponentType<{ data: ResumeData; accentColor: string }>
> = {
...
const TemplateThumbnail = ({
  ...
  return (
    <button
      onClick={onClick}
      className={`relative rounded-xl overflow-hidden border-2 transition-all duration-200 hover:scale-[1.02] hover:shadow-lg flex-shrink-0 ${isSelected ? 'border-primary shadow-md' : 'border-border hover:border-primary/50'}`}
    ...
    >
      ...
      >
        <TC data={SAMPLE_RESUME_DATA} accentColor={accentColor} />
      </div>
      {isSelected && (
            ...
            <div className='grid grid-cols-5 gap-1.5 mb-3'>
              {ACCENT_COLORS.map((c) => (
                <button
                  ...
                  title={c.name}
                  className={`w-9 h-9 rounded-lg border-2 transition-all hover:scale-110 ${selectedColor === c.value ? 'border-white scale-110 ring-2 ring-primary/50 shadow-md' : 'border-transparent hover:border-white/60'}`}
                  style={{ backgroundColor: c.value }}
                >
                  {selectedColor === c.value && (
                    <Check className='m-auto h-4 w-4 text-white drop-shadow' />
                  )}
                </button>
              ))}
            </div>
