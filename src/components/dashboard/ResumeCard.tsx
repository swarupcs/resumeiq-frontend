...
export const ResumeCard = ({
  ...
  >
    <div
      ...
      style={{
        background:
          'linear-gradient(90deg, hsl(var(--primary)), hsl(271 76% 53%))',
        opacity: 0.6,
      }}
    />
    <div className='p-5'>
      <div className='flex items-center justify-between mb-4'>
        <Badge
          ...
          className={`text-xs gap-1 rounded-lg px-2.5 py-1 font-medium ${resume.isPublic ? 'bg-green-500/10 text-green-500 border border-green-500/20' : 'bg-secondary text-muted-foreground border border-border'}`}
        >
          {resume.isPublic ? (
            <>
      ...
      <h3 className='font-display font-bold text-foreground mb-1 line-clamp-1 text-base'>
        {resume.title}
      </h3>
      {resume.template && (
      ...
      <div className='flex items-center gap-3 text-xs text-muted-foreground mb-3'>
        <span className='flex items-center gap-1'>
          ...
          {(resume.views ?? 0).toLocaleString()}
        </span>
        <span className='flex items-center gap-1'>
          <Download className='h-3 w-3' />
          {(resume.downloads ?? 0).toLocaleString()}
        </span>
      ...
      <div className='flex gap-2 opacity-0 group-hover:opacity-100 transition-all duration-200 translate-y-1 group-hover:translate-y-0'>
        <button
          ...
          className='flex-1 py-2 px-3 rounded-xl bg-primary/10 hover:bg-primary/20 text-primary text-xs font-medium transition-colors flex items-center justify-center gap-1.5 border border-primary/20'
        >
        ...
        <button
          ...
          className='flex-1 py-2 px-3 rounded-xl bg-secondary hover:bg-primary/10 hover:text-primary text-xs font-medium text-foreground transition-colors flex items-center justify-center gap-1.5 border border-border'
        >
