      ...
      <DialogContent className='sm:max-w-md'>
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={isPending}
            onClick={() =>
              deleteResume(resume._id, { onSuccess: () => onClose() })
            }
          >
            {isPending && <Loader2 className='mr-2 h-4 w-4 animate-spin' />}
            Delete
...
export const UploadResumeModal = ({
  ...
  const extractText = async (pdfFile: File): Promise<string> => {
    ...
    return pages
      .flatMap((c) => c.items.map((item) => ('str' in item ? item.str : '')))
      .join(' ');
  };
          ...
          <div className='space-y-4 py-4'>
            <div className='flex gap-2.5 p-3 rounded-lg bg-blue-500/5 border border-blue-500/20'>
              ...
              <div className='text-xs text-muted-foreground leading-relaxed'>
                Go to your LinkedIn profile, select all text (Ctrl+A), copy it,
                and paste it below. Include About, Experience, Education, and
                Skills sections.
              </div>
            </div>
            <div className='space-y-2'>
            ...
            <div className='space-y-2'>
              <div className='flex items-center justify-between'>
                ...
                <span
                  className={`text-xs ${linkedInText.length < 50 ? 'text-muted-foreground' : 'text-green-500'}`}
                >
                  {linkedInText.length} chars{' '}
                  {linkedInText.length < 50
                    ? `(need ${50 - linkedInText.length} more)`
                    : '✓'}
                </span>
              </div>
              <textarea
...
export const ResumeAnalyticsModal = ({
  open,
  resume,
  onClose,
}: {
  ...
  const total = views + downloads,
    rate = views > 0 ? Math.round((downloads / views) * 100) : 0;
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className='sm:max-w-md'>
        <DialogHeader>
        ...
        <div className='py-4 space-y-4'>
          ...
          <div className='grid grid-cols-2 gap-3'>
            {[
              { label: 'Total interactions', value: total },
              { label: 'Download rate', value: `${rate}%` },
            ].map(({ label, value }) => (
              <div
                key={label}
          ...
          {!resume.isPublic && (
            <div className='flex gap-2 p-3 rounded-lg bg-amber-500/5 border border-amber-500/20'>
              ...
              <p className='text-xs text-muted-foreground'>
                This resume is private. Make it public to start tracking views
                from shared links.
              </p>
            </div>
          )}
