...
import {
  Eye, EyeOff, Sparkles, Check, Loader2, ArrowRight,
} from 'lucide-react';
...
const Signup = () => {
  ...
  const strengthColors   = ['bg-destructive', 'bg-amber-500', 'bg-primary', 'bg-green-500'];
  ...
  const handleSubmit = (e) => {
    ...
    signup(formData, {
      onSuccess: () =>
        // Redirect to verify-email page with the email in state so we can
        // show it on the page and offer resend
        ...
        <div className='hidden lg:flex lg:w-1/2 relative overflow-hidden'>
          ...
          <div className='relative z-10 flex flex-col justify-center px-12 xl:px-16'>
            ...
            <p className='text-base text-muted-foreground max-w-sm mb-10 leading-relaxed'>
              Create a professional resume in minutes with AI-powered tools and expert templates.
            </p>
        ...
        <div className='w-full lg:w-1/2 flex items-center justify-center px-6 py-12'>
          <div className='w-full max-w-md'>
            ...
            <form onSubmit={handleSubmit} className='space-y-4'>
              ...
              <div className='space-y-1.5'>
                <Label htmlFor='emailId' className='text-sm font-medium'>Email Address</Label>
                <Input
                  id='emailId' name='emailId' type='email' placeholder='john@example.com'
                    ...
                    <div className='flex gap-1.5'>
                      {[...Array(3)].map((_, i) => (
                        <div key={i} className={`h-1 flex-1 rounded-full transition-all duration-300 ${
                          i < passwordStrength ? strengthColors[passwordStrength] : 'bg-secondary'
                        }`} />
                      ))}
                      {passwordStrength > 0 && (
                        <span className={`text-xs font-medium ml-1 ${
                          passwordStrength === 3 ? 'text-green-500' :
                          passwordStrength === 2 ? 'text-primary' : 'text-amber-500'
                        }`}>
              ...
              >
                {isPending
                  ...
                  : <>Create Free Account <ArrowRight className='w-4.5 h-4.5 transition-transform group-hover:translate-x-0.5' /></>
                }
              </button>
            </form>
