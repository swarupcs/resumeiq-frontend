...
const ResumeBuilder = () => {
  ...
  const serverResume = useMemo(() => {
    ...
    return {
      ...
      experience: (resume.experience ?? []).map((exp) => ({
        ...
        description: exp.description ?? '', is_current: exp.is_current ?? false,
      })),
      ...
      project: (resume.project ?? []).map((proj) => ({
        name: proj.name ?? '', type: proj.type ?? '', description: proj.description ?? '',
      })),
  ...
  const {
    state: localOverrides, setState: setLocalOverrides,
  ...
  const handleShare = () => {
    ...
    if (navigator.share) { navigator.share({ url, title: resumeData.title }); }
        ...
        <div className='grid lg:grid-cols-12 gap-8'>
            ...
            <div className='bg-card rounded-xl shadow-sm border border-border p-6 pt-4 relative overflow-hidden'>
                ...
                <div className='flex items-center gap-2'>
                  ...
                  <div className='flex items-center gap-1'>
                    <Button variant='ghost' size='icon' onClick={undo} disabled={!canUndo}
                      className='h-8 w-8 text-muted-foreground hover:text-foreground' title='Undo (Ctrl+Z)'>
                      <Undo2 className='size-3.5' />
                    </Button>
                    <Button variant='ghost' size='icon' onClick={redo} disabled={!canRedo}
                      className='h-8 w-8 text-muted-foreground hover:text-foreground' title='Redo (Ctrl+Y)'>
              ...
              <div className='flex justify-center gap-2 mb-6'>
                {sections.map((section, index) => {
                  ...
                  return (
                    <button key={section.id} onClick={() => setActiveSectionIndex(index)}
              ...
              <div className='space-y-6 min-h-[400px]'>
                ...
                {sections[activeSectionIndex].id === 'projects' && (
                  <ProjectForm data={resumeData.project}
                    onChange={(d) => setResumeData((p) => ({ ...p, project: d }))} />
                )}
              ...
              <Button onClick={handleManualSave} disabled={autosaveStatus === 'saving'}
          ...
          <div className='lg:col-span-7 space-y-4'>
            <div className='flex items-center justify-end gap-2 flex-wrap'>
              ...
              <Button variant='outline' size='sm' onClick={changeResumeVisibility}
