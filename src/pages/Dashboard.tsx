...
import { CreateResumeModal, UploadResumeModal, EditResumeModal, DeleteConfirmDialog, LinkedInImportModal, ResumeAnalyticsModal } from '@/components/dashboard/Modals';
import Navbar from '@/components/Navbar';
import { toast } from 'sonner';
import { useUserResumes } from '@/hooks/user/useUserResumes';
import { useSelector } from 'react-redux';
import { selectCurrentUser } from '@/features/authSlice';
import { useDuplicateResume, useToggleResumeVisibility } from '@/hooks/resume/index';
...
const Dashboard = () => {
  ...
  const handleToggleVisibility = (resume: ResumeListItem) => {
    toggleVisibility(resume._id, {
      onSuccess: () => void queryClient.invalidateQueries({ queryKey: ['userResume'] }),
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="relative overflow-hidden border-b border-border bg-card/40">
          ...
          <div className="container mx-auto px-4 py-8 relative z-10">
            <div className="flex items-center justify-between gap-4 flex-wrap">
              <div>
                <p className="text-sm text-muted-foreground font-medium mb-1">{greeting()}, {user?.firstName ?? 'there'} 👋</p>
                <h1 className="font-display text-2xl sm:text-3xl font-black text-foreground tracking-tight">My Resumes</h1>
            ...
            <div className="container mx-auto px-4">
              <div className="flex gap-3 items-center py-3">
                <div className="relative flex-1 max-w-sm">
                  ...
                  <Input placeholder="Search resumes..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-9 h-9 text-sm bg-secondary/50 rounded-xl" />
                </div>
                ...
                <div className="flex rounded-xl border border-border overflow-hidden">
                  {(['grid', 'list'] as const).map((mode) => (
                    <button key={mode} onClick={() => setViewMode(mode)} className={`p-2 transition-colors ${viewMode === mode ? 'bg-primary text-primary-foreground' : 'bg-secondary/50 text-muted-foreground hover:bg-secondary'}`}>
        ...
        <main className="container mx-auto px-4 py-8">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 px-4">
              <div className="w-28 h-28 rounded-3xl bg-gradient-to-br from-primary/20 to-purple-600/20 border border-primary/20 flex items-center justify-center mb-8"><FileText className="h-12 w-12 text-primary" /></div>
            ...
            <div className={viewMode === 'grid' ? 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5' : 'flex flex-col gap-3'}>
              {filteredResumes.map((resume) => (
                <ResumeCard
                  ...
                  onRename={(r) => { setSelectedResume(r); setRenameModalOpen(true); }}
                  onDelete={(r) => { setSelectedResume(r); setDeleteDialogOpen(true); }}
                  onClick={(r) => navigate(`/app/builder/${r._id}`)}
                  onShare={handleShare}
                  onDownload={(r) => navigate(`/app/builder/${r._id}?download=true`)}
                  onPreview={(r) => navigate(`/preview/${r._id}`)}
                  onToggleVisibility={handleToggleVisibility}
                  onDuplicate={(r) => duplicateResume(r._id)}
                  onAnalytics={(r) => { setSelectedResume(r); setAnalyticsModalOpen(true); }}
                />
              ))}
            </div>
          )}
      ...
      <EditResumeModal open={renameModalOpen} resume={selectedResume} onClose={() => { setRenameModalOpen(false); setSelectedResume(null); }} />
      <DeleteConfirmDialog open={deleteDialogOpen} resume={selectedResume} onClose={() => { setDeleteDialogOpen(false); setSelectedResume(null); }} />
      <ResumeAnalyticsModal open={analyticsModalOpen} resume={selectedResume} onClose={() => { setAnalyticsModalOpen(false); setSelectedResume(null); }} />
    </div>
  );
};
