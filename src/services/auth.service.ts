...
export const authService = {
  signup: async (userData: SignupPayload): Promise<ApiResponse<{ user: User }>> => {
    try {
      const { data } = await signupApi(userData);
      toast.success('Account created! Please check your email to verify.');
      return data as ApiResponse<{ user: User }>;
    } catch (error) { return handleApiError(error, 'Signup failed'); }
  },

  signin: async (userData: SigninPayload): Promise<ApiResponse<{ user: User }>> => {
    try {
      const { data } = await signinApi(userData);
      return data as ApiResponse<{ user: User }>;
    } catch (error) { return handleApiError(error, 'Signin failed'); }
  },

  logout: async (): Promise<void> => {
    try { await logoutApi(); } catch { /* swallow */ }
  },
  ...
  verifyEmail: async (token: string): Promise<ApiResponse<null>> => {
    ...
    } catch (error) { return handleApiError(error, 'Email verification failed'); }
  },
  ...
  resendVerification: async (emailId: string): Promise<ApiResponse<null>> => {
    try {
      ...
      return data as ApiResponse<null>;
    } catch (error) { return handleApiError(error, 'Failed to resend verification email'); }
  },
  ...
  resetPassword: async (token: string, password: string): Promise<ApiResponse<null>> => {
    try {
      ...
      return data as ApiResponse<null>;
    } catch (error) { return handleApiError(error, 'Failed to reset password'); }
  },
};
