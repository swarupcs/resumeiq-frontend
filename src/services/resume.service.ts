...
export const resumeService = {
  getResumeById: async (
    resumeId: string,
  ): Promise<ApiResponse<{ resume: ResumeData }>> => {
    try {
  ...
  getPublicResumeById: async (
    ...
    try {
      ...
      return handleApiError(error, 'Failed to fetch public resume');
    }
  },

  createResume: async (
    title: string,
  ): Promise<ApiResponse<{ resume: ResumeListItem }>> => {
    try {
      const { data } = await createResumeApi({ title });
      return data as ApiResponse<{ resume: ResumeListItem }>;
    } catch (error) {
      return handleApiError(error, 'Failed to create resume');
    }
  },
  ...
  updateResume: async ({
    ...
    image,
    removeBackground,
  }: UpdateResumePayload): Promise<ApiResponse<{ resume: ResumeData }>> => {
    try {
      ...
      return handleApiError(error, 'Failed to update resume');
    }
  },
  ...
  updateResumeTitle: async (
    ...
    try {
      ...
      return handleApiError(error, 'Failed to update title');
    }
  },

  toggleResumeVisibility: async (
    id: string,
  ): Promise<ApiResponse<{ resume: ResumeListItem }>> => {
    try {
      ...
      return handleApiError(error, 'Failed to toggle visibility');
    }
  },

  exportResumePdf: async (
    resumeId: string,
    fullName: string,
  ...
  duplicateResume: async (
    ...
    try {
      ...
      return handleApiError(error, 'Failed to duplicate resume');
    }
  },
  ...
  restoreVersion: async (
    ...
    try {
      ...
      return handleApiError(error, 'Failed to restore version');
    }
  },
};
